...
**Impact:** Smart & Street's research (313 hiring decisions analyzed) found that managers using scorecards to define and evaluate against role outcomes hired "A players" (performers in the top 10% for the role) 90% of the time, compared to 26% using traditional credential-and-interview methods; McKinsey's 2021 talent research found that organizations with clearly defined role outcomes rather than vague job descriptions reduced time-to-productivity for new hires by 35%; Geoff Smart's follow-on research (500+ executive hires) found that 90% of hiring failures trace to misaligned expectations of what success in the role looks like — a gap the scorecard closes before the hire
**Why best:** Job descriptions are marketing documents — they attract candidates; scorecards are performance standards — they define what success requires; traditional interviews evaluate whether a candidate is likable, impressive, and experienced; scorecard-based interviews evaluate whether a candidate has the track record of accomplishing the specific outcomes the role requires; the scorecard replaces subjective "gut feel" hiring with evidence of past performance on the dimensions that matter
...
### 1. Write the mission: one sentence describing what this role is for
...
### 2. Define 3–8 outcomes — what must be accomplished in the first year
...
- What will be true by when
- How you will know if it is true (the measure or observable evidence)
...
### 3. Define competencies — how the outcomes will be achieved
...
### 4. Define cultural requirements
...
### 5. Calibrate the scorecard with all interviewers before the search begins
...
### 6. Score candidates against the scorecard — not against each other

After each interview, score the candidate against the scorecard:
- For each outcome: does the candidate's track record make you confident they can achieve this? (1–4 scale)
- For each competency: what specific evidence supports or undermines this competency?
...
- Write the mission before the outcomes — if you cannot state why the role exists in one sentence, you are not ready to define the scorecard
- Outcomes first, then competencies — competencies are derived from what the outcomes require, not from a generic HR library
- No more than 8 outcomes — more than 8 creates an interview that is too diffuse to probe any outcome well
- Every interviewer scores against the same scorecard — if each interviewer uses their own informal criteria, you are running multiple different interviews, not one structured process
- Do not substitute the scorecard with the job description — job descriptions attract; scorecards select; they serve different purposes and must not be conflated

## Common Mistakes

- **Writing tasks instead of outcomes**: "manages the engineering team" is not an outcome; "builds a team capable of shipping 2× the current release cadence within 9 months" is.
- **Generic competencies not tied to this role**: "strong communication" and "team player" describe almost every job; competencies must be specific to what this role requires.
- **Scorecard written by one person without calibration**: if the hiring manager writes the scorecard but interviewers evaluate against their own implicit criteria, the scorecard has no effect on the decision.
- **Compromising on the scorecard to hire fast**: hiring someone who meets 60% of the scorecard fills the seat and fails the team; "we'll see if they can grow into it" is a prediction, not a plan.
- **Different scorecard for different candidates**: if you modify the outcomes or competencies during the search based on who you've seen, you're optimizing for available candidates rather than the role's actual requirements.

## When NOT to Use

- For high-volume, low-complexity roles where the job is well-defined and outcomes are identical across all role-holders — in these cases, a standardized job specification and structured interview rubric is sufficient; the full scorecard framework is best suited for roles where outcomes vary based on organizational context and moment.