...
- **Amazon Leadership Principles** (14 principles, published publicly): All hiring decisions, promotion decisions, and performance actions reference these explicitly. The rules exist before any specific employee situation. Jeff Bezos insisted that they be written, published, and stable — not subject to managerial reinterpretation in the moment.
- **Netflix Culture Deck** (published 2009, 20M+ views): Reed Hastings published explicit behavior standards, performance expectations, and consequences before any individual situation arose. The deck was designed specifically so that managers couldn't invoke discretion — employees could reference the published rule themselves.
- **Google Engineering Culture**: ADR (Architecture Decision Records) and internal design docs make decisions written, published, and accessible. Engineers appeal to the rule, not the person.
- **Procedural justice research** (Leventhal 1980; Tyler & Lind 1992; meta-analysis by Colquitt 2001): The most replicated finding in organizational psychology — employee compliance and trust are most strongly predicted by whether rules were known in advance and applied consistently. Organizations with published, pre-stated norms have higher compliance rates, lower grievance rates, and faster conflict resolution than those relying on discretionary judgment.
...
1. **Identify where discretion currently lives.** Map every point where an individual (manager, enforcer, executive) currently makes judgment calls without reference to a written standard. Ask: "If this person acted differently tomorrow, would anyone have grounds to object?" If no — the rule is implicit and the power is personal.

2. **Draft the rule before the specific case arrives.** Write the standard while no particular case is pending. Rules drafted in response to specific situations are reverse-engineered to produce a predetermined outcome — which is still discretion wearing a procedural costume. If a case is already pending, recuse the rule from that case and apply prospectively.

3. **State the rule with enough specificity to be falsifiable.** A rule that cannot be violated cannot be enforced. "We treat people with respect" is not a rule — it's an aspiration. "Performance reviews are completed within 30 days of the review period" is a rule. For each standard, identify: what specific action or outcome does it require or prohibit? What constitutes a violation?

4. **Publish in a form that is permanent and accessible to those governed.** Bronze cauldrons were permanent (hard to alter) and public (displayed at the city gate). The modern equivalent: written policy in a location where the governed population can find and reference it independently. Accessibility matters — a rule buried in a legal document no one reads is discretion with paperwork.

5. **Apply the rule consistently to the first cases that test it.** The first enforcement event establishes whether the rule is real. If the first violation is excused for relational or political reasons, the rule has been revealed as advisory rather than binding — and the population now knows that rule-exception decisions depend on relationship, not rule. Apply uniformly to early cases, especially when uniform application is costly.

6. **Create a stable revision process for the rules themselves.** Published rules are not immutable — they should be updated when conditions change. But revision must be prospective (announced before taking effect), deliberate (not in response to a pending case), and itself governed by a published process. Arbitrary revision of published rules is equivalent to having no rules.

7. **Eliminate parallel informal norms.** If the published rule says one thing and the informal norm (what managers actually do) says another, the informal norm is the real rule — and it has all the problems of discretion. Either revise the published rule to match practice, or eliminate the informal norm through enforcement.

## Rules

- Never draft a rule in response to a specific pending case. Prospective rules only — if a case is already live, defer the rule to future situations.
- Publication to those who enforce is not the same as publication to those governed. The rule must be accessible to the people whose behavior it regulates, not only to the team enforcing it.
- Consistent first-case enforcement is not negotiable. Discretion in the first enforcement event permanently signals that rule-exception decisions are relational. There are no "soft" first cases.
- Rules with undefined violation conditions are guidelines, not rules. If a rule cannot be violated, it cannot be enforced — and it provides no predictability benefit.
...
- **Publishing rules after cases arise:** Retroactively applying a rule to a case that was already live is enforcement by reverse-engineered justification. The rule provides no predictability benefit and damages trust more than discretion alone.
- **Making the rules too abstract to be violated:** "We value integrity" is not a rule. It cannot be violated and therefore cannot be enforced. Rules must specify observable behavior or outcomes — "Conflicts of interest must be disclosed to the compliance team before participating in any decision involving the conflicting party."
- **Publishing but not enforcing:** A published rule that is not enforced is worse than no rule — it signals that the publication process is theater, that the actual standard is still whoever holds power, and that the published document cannot be relied upon. Unenforced rules erode trust in all published rules.
- **Allowing informal norms to persist alongside published rules:** If the informal practice diverges from the published rule, the informal practice is the real governance system. Close the gap by either enforcing the published rule or updating it to match practice.
- **Treating rules as immutable:** Published rules should be updated when conditions change — but prospectively and through a deliberate, published process. Organizations that treat rule revision as failure avoid necessary updates and accumulate rules that no longer reflect practice.