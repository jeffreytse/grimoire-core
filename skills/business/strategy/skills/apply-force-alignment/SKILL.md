...
1. **Enumerate the major forces in play.** For the domain of your strategy, identify forces that are large, sustained, and not under your control. Categories to examine:
   - Technology forces (adoption curves, cost curves, capability improvements)
   - Market forces (demand growth, customer behavior shifts, purchasing pattern changes)
   - Regulatory/policy forces (tightening or loosening regulation, new standards)
   - Demographic forces (population shifts, generational behavior changes)
   - Economic forces (interest rate environments, cost structure shifts)
   - Competitive forces (industry structure changes, platform consolidation)
...
2. **Classify each force as tailwind or headwind for your strategy.** A tailwind works in the same direction as your strategy — it does work on your behalf and your strategy captures its energy. A headwind opposes your strategy — your strategy must overcome it or it accumulates cost over time. Be precise: the same force can be a tailwind for one strategy and a headwind for an alternative.

3. **Score the force-alignment profile.** Count tailwinds and headwinds. Categorize their magnitude (major vs. minor). A force-misaligned strategy has major headwinds and few tailwinds. A force-aligned strategy has major tailwinds and manageable headwinds. No strategy achieves perfect alignment — the question is the profile.
...
5. **Where multiple strategy options exist, prefer the force-aligned one.** When evaluating strategy alternatives, use force-alignment as a primary decision criterion, not a secondary check. Two strategies with similar expected value under current conditions have very different expected value under force dynamics — the aligned strategy compounds; the opposed strategy erodes.

6. **Detect force reversals early.** Major forces do reverse — technology becomes commoditized, regulation shifts, demographics change. Monitor leading indicators of force reversal (Christensen's strategic inflection points, Grove's "10x" signals). When a force reversal is detected, treat it as a strategy invalidation event requiring force-alignment re-audit, not a temporary perturbation requiring tactical adjustment.

7. **Design the business model to capture force energy.** The most durable strategies are not merely aligned with forces but designed to accelerate force capture — so that as the force grows stronger, the strategy benefits more. AWS was not merely aligned with cloud migration; it was designed so that every enterprise that migrated to cloud increased AWS's capabilities, scale, and cost advantage, which accelerated further migration.

## Rules

- Force opposition is not automatically wrong — sometimes opposing a force is the correct position. But it requires an explicit credible mechanism for overcoming the force and an explicit resource budget for the sustained cost of opposition. Without these, it is not a strategy: it is an assumption.
- Large forces dominate long-run outcomes; execution quality dominates short-run outcomes. A perfectly executed strategy opposing a major force will eventually lose to a mediocrely executed strategy aligned with it.
- "We are better than the force" is not a mechanism. Organizational excellence does not cancel external forces; it affects execution quality within the force landscape.
- Do not use this to rationalize passivity. Force alignment is active: it requires early positioning, mechanism design, and continuous monitoring. "We are riding a tailwind" without a specific positioning strategy captures little of the force.
...
- **Treating force-alignment analysis as scenario planning:** Force alignment audits the current landscape; scenario planning maps future landscapes. Doing one doesn't substitute for the other.
- **Identifying forces but not taking force-alignment into account in the decision:** The analysis has value only if it changes the decision. If force-alignment shows major headwinds and the strategy proceeds without credible mechanisms, the analysis was not applied.
- **Calling execution quality a mechanism:** "We will execute better than competitors" is not a mechanism for overcoming a structural force. Specify what specifically you will do differently, with what resources, to produce what measurable result.
- **Over-indexing on current forces without monitoring for reversals:** Forces change. A strategy perfectly aligned today can become misaligned within 3–5 years if the force changes direction. Build monitoring into the strategy, not just the initial analysis.
- **Treating all forces as equally significant:** A major force (e.g., AI capability cost-reduction curve) and a minor force (e.g., a single regulatory update) are not equivalent. Calibrate the weight of each force by its magnitude and rate of change.